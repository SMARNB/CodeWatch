...
const NotificationDetailsPage = () => {
  ...
  const handlePostFeedback = async () => {
    ...
    try {
      ...
      if (response.ok) {
        setToast('Posted - added to Reports');
  ...
  // Capture the violator card (snapshot + violation details) as a PDF for the emailed report.
  const captureViolationPdf = async () => {
    ...
    try {
      ...
      pdf.addImage(imgData, 'JPEG', 10, 10, imgW, imgH, 'violation', 'FAST');
      return { report_id: null, filename: 'violation_report.pdf', base64: pdf.output('datauristring') };
    } catch (e) {
      console.error('Violation PDF capture failed', e);
      return null;
    }
  };
    ...
    <div className="min-h-screen bg-[#f5f3ff] relative overflow-hidden font-sans">
      ...
      <div className="page-container relative w-full z-10" style={{ display: 'flex', gap: '20px', padding: '20px', maxWidth: '1400px', margin: '0 auto' }}>
              ...
              <div className="flex justify-between items-center w-full">
                ...
                <div className="flex items-center gap-2" style={{ marginRight: '10px' }}>
                  <button
                    ...
                    className="px-6 h-[48px] border border-[#3f4299] text-[#3f4299] rounded-[8px] text-[14px] font-semibold hover:bg-[#3f4299]/5 transition-colors focus:outline-none"
        ...
        <div className="right-column" style={{ flex: '0 0 32%', display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div ref={reportRef} className="bg-white rounded-lg shadow-sm border border-gray-200 flex flex-col text-center" style={{ padding: '16px' }}>
      ...
      {sendOpen && (
        <SendReportModal
          ...
          getClientPdf={captureViolationPdf}
          showReportPicker={false}
          violationData={{
            type: notification?.violation_details?.type || notification?.title || 'Violation',
            personName: personInfo?.name || 'Unknown Person',
            camera: activeCamera?.name || notification?.camera_details?.name || notification?.camera_id || 'Unknown',
            personId: notification?.person_id || personInfo?.id || null,
          }}
        />
      )}
    </div>
