...
const GenerateAnalyticsPage = () => {
  ...
  // Camera filter (single-select via the thumbnails)
  const [selectedCamera, setSelectedCamera] = useState(null);

  const handleSelectCamera = (cam) => {
    setSelectedCamera(prev => (prev && prev.camera_id === cam.camera_id ? null : cam));
  };
  ...
  useEffect(() => {
    fetch(`/api/analytics-filters/?user=${encodeURIComponent(localStorage.getItem('userEmail') || '')}`)
      .then(res => res.json())
      .then(data => {
  ...
  const generateAnalyticsData = (realCounts) => {
    ...
    // Build a month-over-month % + 6-month sparkline from a [{month,count},...] series.
    // No real activity -> return blank so the card shows the value only (no fake trend).
    const cardMetric = (series) => {
      const counts = Array.isArray(series) ? series.map(p => (p && typeof p.count === 'number' ? p.count : 0)) : [];
      if (counts.length < 2) return { pct: '', spark: [] };
      ...
      return { pct, spark: counts.slice(-6) };
    };

    const nonViolatorsTrend = cardMetric(realCounts.monthlyNonViolators);
    const violatorsTrend    = cardMetric(realCounts.monthlyViolators);
    const unauthorizedTrend = cardMetric(realCounts.monthlyUnauthorized);
    const visitorsTrend     = cardMetric(realCounts.monthlyVisitors);

    const statCardsData = [
      { title: 'Non-Violators', value: nonViolators.toString(), percentageChange: nonViolatorsTrend.pct, miniChartData: nonViolatorsTrend.spark },
      { title: 'Violators',     value: violators.toString(),    percentageChange: violatorsTrend.pct,    miniChartData: violatorsTrend.spark },
      { title: 'Unauthorized',  value: unauthorized.toString(), percentageChange: unauthorizedTrend.pct, miniChartData: unauthorizedTrend.spark },
      { title: 'Visitors',      value: visitors.toString(),     percentageChange: visitorsTrend.pct,     miniChartData: visitorsTrend.spark },
    ];
  ...
  const createReportDescription = (filters, analyticsData) => {
    ...
    if (filters.userType && filters.userType.length > 0) {
      parts.push(`Person Type: ${filters.userType.map(formatOptionText).join(', ')}`);
    }
...
------------------------------------------------
Filters Applied: 
Camera: ${selectedCamera ? (selectedCamera.name || selectedCamera.camera_id) : 'All'}
...
• Visitors: ${analyticsData.statCardsData[3].value}
      ...
      setAlertData({ isOpen: true, message: "Network Error: Is the Backend running?" });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#f2f3ff] relative overflow-hidden">
      ...
      <div className="relative w-full" style={{ paddingTop: '100px', paddingLeft: '100px', paddingRight: '100px' }}>
        ...
        {/* Active camera filter indicator */}
        {selectedCamera && (
          <div className="mb-6" style={{ marginBottom: '20px' }}>
            <span
              className="inline-flex items-center gap-2 h-[36px] rounded-full bg-[#3f4299] text-white text-[13px] font-medium"
              style={{ fontFamily: "'Open Sans', sans-serif", paddingLeft: '14px', paddingRight: '8px' }}
            >
              Showing data for: {selectedCamera.name || selectedCamera.camera_id}
              <button
                onClick={() => setSelectedCamera(null)}
                className="flex items-center justify-center w-[22px] h-[22px] rounded-full bg-white/20 hover:bg-white/30 transition-colors"
