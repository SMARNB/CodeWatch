  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok && data.status === 'success') {
        ...
        // Show the exact reason from the backend (not registered / incorrect password / etc.)
  ...
  return (
    <div className="bg-[#f2f3ff] relative size-full min-h-screen flex items-center justify-center" data-name="Admin Log In">
      ...
      <div className="absolute h-[449px] left-1/2 top-[calc(50%-0.5px)] translate-x-[-50%] translate-y-[-50%] w-[388px]" data-name="Log In">
        ...
        <form onSubmit={handleSubmit} className="absolute inset-0 size-full">
          {/* Email Input */}
          ...
          <input
            ...
            className="w-full h-full pr-4 text-[14px] text-black bg-transparent border-none outline-none"
        ...
        <div className="absolute bg-white border border-[#bab6b6] border-solid h-[48px] left-[30px] rounded-[8px] top-[255px] w-[328px]">
          <input
            ...
            className="w-full h-full pr-[40px] text-[14px] text-black bg-transparent border-none outline-none"
            style={{ paddingLeft: '10px' }}
            required
          />
          <button
