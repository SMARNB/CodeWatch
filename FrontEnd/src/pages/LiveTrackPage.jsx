...
const LiveTrackPage = () => {
  ...
  // Capture the person card (snapshot + details) as a PDF for the emailed report.
  const captureViolationPdf = async () => {
    ...
    try {
      ...
      pdf.addImage(imgData, 'JPEG', 10, 10, imgW, imgH, 'person', 'FAST');
      return { report_id: null, filename: 'tracking_report.pdf', base64: pdf.output('datauristring') };
    } catch (e) {
      console.error('Tracking PDF capture failed', e);
      return null;
    }
  };
      ...
      {sendOpen && (
        <SendReportModal
          ...
          violationData={{
            ...
            camera: activeCamera?.name || 'Unknown',
            personId: personInfo?.id || personId || null,
          }}
        />
      )}
    </div>
