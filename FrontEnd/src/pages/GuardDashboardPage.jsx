...
const GuardDashboardPage = () => {
  ...
  const handleCheckout = async (id) => {
    try {
      const response = await fetch(`/api/visitors/checkout/${id}/`, {
        method: 'POST',
      });
      if (response.ok) {
        fetchActiveVisitors();
        ...
        setAlertData({ isOpen: true, message: 'Failed to checkout visitor' });
      }
    } catch (error) {
  ...
  const handleExtend = async (id) => {
    try {
      const response = await fetch(`/api/visitors/extend/${id}/`, {
        method: 'PATCH',
        headers: {
      ...
      if (response.ok) {
        ...
        setAlertData({ isOpen: true, message: 'Failed to extend visitor' });
      }
    } catch (error) {
  ...
  return (
    <div className="font-sans pb-10">
      <div className="w-full mx-auto" style={{ paddingLeft: 'clamp(16px, 7vw, 150px)', paddingRight: 'clamp(16px, 7vw, 150px)', marginTop: '40px' }}>
