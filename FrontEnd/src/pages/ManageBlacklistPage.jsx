...
const ManageBlacklistPage = () => {
  ...
  const fetchBlacklist = async () => {
    try {
      setLoading(true);
      const u = encodeURIComponent(localStorage.getItem('userEmail') || '');
      const [blacklistRes, peopleRes] = await Promise.all([
        fetch(`/api/blacklist/?user=${u}`),
        fetch(`/api/people-db/?user=${u}`)
      ]);
  ...
  const filteredBlacklist = blacklist.filter(entry => {
    ...
    return (entry.person_name || '').toLowerCase().includes(q) ||
           (entry.employee_id || '').toLowerCase().includes(q);
  });
  ...
  return (
    <div className="min-h-screen bg-[#f2f3ff] relative overflow-hidden font-['Open_Sans']">
      ...
      {/* Main Content */}
      <div className="relative w-full pb-12 flex gap-8" style={{ paddingLeft: 'clamp(16px, 5vw, 64px)', paddingRight: 'clamp(16px, 5vw, 64px)', marginTop: '100px' }}>
        <div className="flex-1 min-w-0">

          {/* Stats Bar & Header */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-[1fr_1fr_1fr_290px] gap-[31px] w-full" style={{ marginBottom: '30px' }}>
            ...
            <div className="flex flex-col justify-center items-stretch sm:items-end">
              <button
                onClick={() => setIsAddModalOpen(true)}
                className="w-full sm:w-[200px] h-[48px] bg-red-600 text-white text-[16px] font-bold rounded-[8px] hover:bg-red-700 transition-colors focus:outline-none focus:ring-2 focus:ring-red-600 focus:ring-offset-2"
