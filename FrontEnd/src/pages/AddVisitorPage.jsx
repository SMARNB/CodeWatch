...
const AddVisitorPage = () => {
  ...
  const startCamera = async () => {
    ...
    try {
      ...
      setAlertData({ isOpen: true, message: "Could not access the camera. Please use file upload instead." });
      setIsCameraOpen(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        setAlertData({ isOpen: true, message: 'Visitor registered successfully!' });
        navigate('/guard/dashboard');
      } else {
        const errData = await response.json();
        setAlertData({ isOpen: true, message: `Error: ${errData.message || 'Failed to register visitor'}` });
      }
    } catch (error) {
      console.error("Submit error:", error);
      setAlertData({ isOpen: true, message: "An error occurred while submitting." });
    } finally {
      setLoading(false);
    }
  };
