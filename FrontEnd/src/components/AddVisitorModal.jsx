...
const AddVisitorModal = ({ onClose }) => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        const res = await fetch('/api/people-db/');
  ...
  const handleSubmit = async () => {
    ...
    if (!formData.profilePicture) newErrors.profilePicture = 'A face photo is required so the camera can recognise the visitor';
    ...
    try {
      ...
      // 2. Handle Response
      if (response.ok) {
        setAlertData({ isOpen: true, message: "✅ Visitor Added Successfully!" });
        if (onClose) onClose();
      } else {
        const errorData = await response.json();
        setAlertData({ isOpen: true, message: "❌ Error: " + (errorData.message || "Failed to add visitor") });
      }
    } catch (error) {
      ...
      >
        ...
        <div className="space-y-5 overflow-y-auto flex-1" style={{ padding: '10px' }}>
          ...
          <div className="flex flex-col items-center">
            ...
            <p className="mt-1 text-xs text-gray-400" style={{ fontFamily: "'Open Sans', sans-serif" }}>Face photo (required)</p>
            <input
              type="file"
          ...
          <div className="relative">
            ...
            {hostSelected ? (
              ...
              <input
                ...
                className={`w-full h-[48px] border rounded-[8px] text-[14px] text-black bg-white outline-none transition-colors placeholder:text-[#bab6b6] ${errors.host ? 'border-red-500 focus:ring-2 focus:ring-red-500 focus:border-red-500' : 'border-[#bab6b6] focus:ring-2 focus:ring-[#3f4299] focus:border-[#3f4299]'}`}
                style={{ fontFamily: "'Open Sans', sans-serif", padding: '10px', marginBottom: '10px' }}
              />
            )}
          ...
          {/* Contact Email (Optional) — for violation notifications */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2" style={{ fontFamily: "'Open Sans', sans-serif" }}>
              Email (Optional)
            </label>
            <input
              ...
              onChange={handleInputChange}
              placeholder="Used to email them about any violations"
              className="w-full h-[48px] border border-[#bab6b6] rounded-[8px] text-[14px] text-black bg-white outline-none transition-colors placeholder:text-[#bab6b6] focus:ring-2 focus:ring-[#3f4299] focus:border-[#3f4299]"
              style={{ fontFamily: "'Open Sans', sans-serif", padding: '10px', marginBottom: '10px' }}
            />
          </div>
