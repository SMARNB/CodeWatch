...
const CameraThumbnails = ({ onSelectCamera, selectedCameraId, autoSelectFirst = true }) => {
        ...
        const fetchCameras = async () => {
            try {
                ...
                if (response.ok) {
                    ...
                    // Select first camera by default (skipped when autoSelectFirst={false})
                    if (isInitialFetch && autoSelectFirst && data.length > 0 && onSelectCamera) {
    ...
    return (
        <div className="w-full">
            ...
            <ScrollArea className="w-full overflow-x-auto border-0 pr-4 py-4 min-h-[170px]">
                <div className="flex gap-[15px] w-full items-center" style={{ paddingLeft: '50px' }}>
                    {cameras.map((cam) => (
                        <CameraThumbnail
                            key={cam.camera_id}
                            camera={cam}
