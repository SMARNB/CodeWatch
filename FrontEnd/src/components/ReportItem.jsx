...
const ReportItem = ({ reportId, description, onUpdate, onDelete, onClick, report, displayNumber }) => {
  const [isDropdownOpen, setIsDropdownOpen] = useState(false);
  const dropdownRef = useRef(null);
  const badge = STATUS_BADGE[(report?.status || '').toLowerCase()] || { label: report?.status || 'New', cls: 'bg-gray-100 text-gray-700' };
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-4">
            <div className="flex-shrink-0">
              <span className="text-sm font-semibold text-[#3f4299]" style={{ fontFamily: "'Open Sans', sans-serif", width: '24px', display: 'inline-block', textAlign: 'center' }}>
            ...
            <div className="flex-1 min-w-0 flex items-center gap-3">
              ...
              <span 
                className={`inline-block rounded-full text-[12px] font-medium shrink-0 ${badge.cls}`} 
              ...
              {report.movement_summary && report.movement_summary.length > 0 && (
                <span 
                  className="inline-block bg-red-100 text-red-800 rounded-full text-[12px] font-medium shrink-0"
                  style={{ padding: '2px 12px' }}
                >
                  1 Violator
                </span>
