  ...
  useEffect(() => {
    const fetchReports = () => {
      try {
        ...
        if (reportsJson) {
          savedReports = JSON.parse(reportsJson);
          // Convert date strings back to Date objects
          savedReports = savedReports.map(report => ({
            ...report,
            date: report.date ? new Date(report.date) : new Date(report.timestamp)
          }));
        ...
        // Include mock reports if no saved reports exist (for initial state)
        if (savedReports.length === 0) {
          const mockReports = [
            {
              id: 'RPT-001',
              description: 'Monthly Security Violation Report - January 2024',
            ...
            {
              id: 'RPT-002',
              description: 'Weekly Access Control Report - Week 3',
            ...
            {
              id: 'RPT-003',
              description: 'Quarterly Compliance Report - Q4 2023',
    ...
    // Listen for storage changes (when new reports are added from other tabs/pages)
    const handleStorageChange = (e) => {
      if (e.key === 'analyticsReports' || !e.key) {
        fetchReports();
      }
    };
    ...
    return () => {
      window.removeEventListener('storage', handleStorageChange);
      window.removeEventListener('reportsUpdated', fetchReports);
    };
  }, []);
